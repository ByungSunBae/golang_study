...
func main() {
	...
	//객체 지향 언어일까? O
	//Go는 전형적인 객체지향의 특징을 가지고 있진않지만, 인터페이스 -> 다형성 지원, 구조체를 클래스 형태의 코딩가능
	//객체지향의 기본 개념 -> Go에서 포함하고있다. -> 객체지향 프로그래밍 언어
	//상태, 메소드 분리해서 정의(결합성 없음)
	//사용자 정의 타입 : 구조체, 인터페이스, 기본타입(int, float, string, ...), 함수
	//구조체 -> 메소드 연결을 통해서 타 언어의 클래스 형식처럼 사용가능(객체 지향)

	//예제1
	bmw := Car{name: "520d", price: 50000000, color: "white", tax: 5000000}
	benz := Car{name: "220d", price: 80000000, color: "black", tax: 8000000}
